...
class DenoiserDataset(Dataset):
    def __init__(self, input_file_path, output_file_path, patch_size, train_flag):
        ...
        self.input_imgs          = natsorted(sorted(glob.glob(self.input_imgs_path + "/*.png"), key=len))

        self.output_imgs_path    = output_file_path
        self.output_imgs         = natsorted(sorted(glob.glob(self.output_imgs_path + "/*.png"), key=len))
    ...
    def __len__(self):
        return len(self.input_imgs_perceptual)

    def __getitem__(self, index):

        input_img            = imageio.imread(self.input_imgs_LR[index])
        input_img            = torch.from_numpy((input_img) / 255.0).permute(2,0,1).float()  
